...
fuzz_target!(|input: Input| {
    ink::env::test::run_test::<::ink::env::DefaultEnvironment, _>(|_| {
        let erc20 = test_utils::misuse_contract_storage(input.initial_supply, input.bob_initial_allowance, input.bob_exploited_allowance, input.storage_location);
        ...
        assert_eq!(allowance, input.bob_initial_allowance, "Fuzzer has found the invalid input");
        Ok(())
    })
    .unwrap();
});
