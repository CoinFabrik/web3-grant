...
In this section we list relevant security-related issues that are typically
introduced during the development of smart contracts in Substrate Ink!. 
Many do generalize to Substrate-based networks, but that need not be the rule.
The list is, of course, non-exhaustive but all of these are very relevant. 
...
other issues. This kind of issues do not block deployments in production 
environments. They should be taken into account and be fixed when possible.
* __Enhancement__: This class relates to issues related to deviations from 
...
We used the Vulnerability Categories above, and common examples of 
vulnerabilities detected within each category in other blockchains, as a 
guideline for finding and developing vulnerable examples of Substrate Ink!
smart contracts.

## A Curated List of Security Vulnerabilities
As a result of our research, we produced seven types of vulnerabilities each
falling under the six following different vulnerability categories (so two
types fall in one category): 
Arithmetic, 
Authorization, 
Denial of Service, 
Reentrancy, and 
